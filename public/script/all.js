 ...
 // 封装的简易验证组件
  function erroDate(s,calback){
    ...
    var erros={
         ...
         erroinsName:'机构品牌名称不能为空',
         erroinsregName:'机构注册名称不能为空',
         erroinsType:'机构类型不能为空'
    }
     ...
     var $tel=$(dom+" .zdTel");
      if($.telReg($tel.val())){
        ...
        var timer=setInterval(function(){
           $(dom+" .regBtn").addClass('click').attr("disabled",true);
          $(dom+" .regBtn").html(time+"秒后重发");
           if(time<0){
           $(dom+" .regBtn").removeClass('click').removeAttr("disabled").html('重新获取验证码');
            clearInterval(timer);
           }
            time-=1;
        },1000);
...
function validateForm(form){
  ...
  var $inisname=$(form+" .zdInstituName");
  var $inisregname=$(form+" .zdRegistNmae");
  var $inistype=$(form+" .zdInstituType");
  var $sub=$(form+" .zdSub");

  $sub.on('click',function(){    
    
    if($inisname.length>0){
      if(!$inisname.val()){
        $inisname.parents(".form-group ").find(".prompt").remove();
        $inisname.focus();
        $inisname.parents(".form-group ").append(erroDate("erroinsName")).addClass('error');
        return false;
      }
      else{
         $inisname.parents(".form-group ").addClass('succ');
      }
    }
    if($inisregname.length>0){
      if(!$inisregname.val()){
        $inisregname.parents(".form-group ").find(".prompt").remove();
        $inisregname.focus();
        $inisregname.parents(".form-group ").append(erroDate("erroinsregName")).addClass('error');
        return false;
      }
      else{
         $inisregname.parents(".form-group ").addClass('succ');
      }
    }
    if($inistype.length>0){
      if($inistype.val()=='0'){
        $inistype.parents(".form-group ").find(".prompt").remove();
        $inistype.focus();
        $inistype.parents(".form-group ").append(erroDate("erroinsType")).addClass('error');
        return false;
      }
      else{
        
         $inistype.parents(".form-group ").addClass('succ');
      }
    }
    if($tel.length>0){
      if(!$.telReg($tel.val())){
     ...
     switch(valText){
      ...
      // #farmed  装裱状态
        ...
        $("#seal,#farmed").addClass('hidden');
         ...
         // enterTip
         $(".long").parents('.form-group').find('.enterTip').remove();
         break;
         case '9'||'11':
